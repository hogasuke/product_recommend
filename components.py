...
def display_stock_status(stock_status):
    ...
    Args:
        stock_status (str): 在庫状況（「残りわずか」、「なし」、「あり」）
    """
    if stock_status == ct.STOCK_STATUS_LOW:
        st.warning(f"{ct.STOCK_ICON_WARNING} {ct.STOCK_MESSAGE_LOW}")
    elif stock_status == ct.STOCK_STATUS_OUT:
        st.error(f"{ct.STOCK_ICON_ERROR} {ct.STOCK_MESSAGE_OUT}")
    elif stock_status == ct.STOCK_STATUS_AVAILABLE:
        st.success(f"{ct.STOCK_ICON_SUCCESS} {ct.STOCK_MESSAGE_AVAILABLE}")
...
def display_product(result):
    ...
    # 在庫状況の表示
    display_stock_status(product.get('stock_status', ct.STOCK_STATUS_AVAILABLE))
